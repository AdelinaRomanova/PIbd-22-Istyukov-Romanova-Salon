...
namespace BeautySalonViewEmployee
{
    ...
    public partial class WindowCosmetics : Window
    {
        ...
        public WindowCosmetics(ICosmeticLogic logic)
        {
            InitializeComponent();
            this._logic = logic;
        }
        ...
        private void LoadData()
        {
            try
            {
                var list = _logic.Read(new CosmeticBindingModel { EmployeeId = id });
                if (list != null)
                {
        ...
        private void buttonDel_Click(object sender, RoutedEventArgs e)
        {
            if (dataGrid.SelectedCells.Count != 0)
            {
                ...
                {
                    int id = (int)((CosmeticViewModel)dataGrid.SelectedCells[0].Item).Id;
                    try
                    {
